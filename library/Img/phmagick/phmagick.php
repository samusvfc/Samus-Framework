...
class phmagick {
    ...
    public function requirePlugin($name, $version=null) {
        ...
        if (key_exists($name, $this->loadedPlugins)) {
            if (!is_null($version)) {
                if (property_exists($this->loadedPlugins[$name], 'version')) {
                    ...
                    if ($this->debug)
                        throw new phMagickException('Plugin "' . $name . '" version =' . $this->loadedPlugins[$name]->version . ' required >= ' . $version);
                }
            }
            return true;
        }
        ...
        if ($this->debug)
            throw new phMagickException('Plugin "' . $name . '" not found!');
        return false;
    }
    ...
    public function execute($cmd) {
        ...
        if ($ret != 0)
            if ($this->debug)
                trigger_error(new phMagickException('Error executing "' . $cmd . '" <br>return code: ' . $ret . ' <br>command output :"' . implode("<br>", $out) . '"'), E_USER_NOTICE);
